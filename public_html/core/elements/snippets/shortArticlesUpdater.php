...
$result = $modx->query("SELECT modx.pagetitle, modx.id, modx.content, k2.title, k2.fulltext, k2.introtext FROM `qtQqv2ATujag_site_content` modx LEFT JOIN cbs_k2_items k2 ON (modx.pagetitle = k2.title) WHERE modx.`template` = 4 AND LENGTH(modx.`content`) < 200");
if (!is_object($result)) {
    return 'No result!';
} else {
